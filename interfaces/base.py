...
lmdbDataset_realBadSet, alignCollate_syn_random_reso, lmdbDataset_realIIIT, lmdbDataset_realForTest, lmdbDataset_svt, lmdbDataset_ic2015
...
class TextBase(object):
    def __init__(self, config, args, opt_TPG=None):
        ...
        if self.args.syn:
            ...
            if self.args.arch == "tsrn":
                ...
                if self.args.test_svt :
                  self.align_collate = alignCollate_realWTLAMask
                  self.load_dataset = lmdbDataset_svt

                  self.align_collate_val = alignCollate_realWTL
                  self.load_dataset_val = lmdbDataset_svt
                elif self.args.test_ic2015 :
                  self.align_collate = alignCollate_realWTLAMask
                  self.load_dataset = lmdbDataset_ic2015

                  self.align_collate_val = alignCollate_realWTL
                  self.load_dataset_val = lmdbDataset_ic2015
                else:
                  self.align_collate = alignCollate_realWTLAMask
                  self.load_dataset = lmdbDataset_real
    ...
    def generator_init(self, iter=-1):
        ...
        if self.args.arch == 'tsrn':
            ...
            image_crit = image_loss.ImageLoss(gradient=self.args.gradient, edge=True, loss_weight=[1, 1e-4, 1e-4])
            ...
            if self.resume is not '':
                ...
                if self.config.TRAIN.ngpu == 1:
                    ...
                    if os.path.isdir(self.resume):
                        ...
                        model.module.load_state_dict(torch.load(self.resume)['state_dict_G'])
                else:
    ...
    def save_checkpoint(self, netG_list, epoch, iters, best_acc_dict, best_model_info, is_best, converge_list, recognizer=None):
        ...
        for i in range(len(netG_list)):
            ...
            if is_best:
                torch.save(save_dict, os.path.join(ckpt_path, 'model_best_' + self.args.test_model + '.pth'))
            else:
